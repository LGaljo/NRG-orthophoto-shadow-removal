# USAGE
# python compare_stcgan_checkpoints_histogram.py
...
def make_prediction(G1, G2, image_path):
    """
    Make a prediction using the ST-CGAN model and return the original image and prediction
    """
    ...
    with torch.no_grad():
        ...
        # Normalize to [-1, 1]
        image = (image - 0.5) / 0.5
        orig_image = (image * 0.5 + 0.5).copy()  # Convert back to [0, 1] for visualization
        ...
        return orig_image, shadow_removal_image
...
def compare_checkpoints():
    ...
    os.makedirs("histogram_comparison_stcgan", exist_ok=True)
    ...
    for checkpoint in MODEL_CHECKPOINTS:
        ...
        # Load the G1 model (shadow detection)
        model_path_G1 = checkpoint["model_path_G1"]
        ...
        # Load the G2 model (shadow removal)
        model_path_G2 = checkpoint["model_path_G2"]
        ...
        for i, shadow_path in enumerate(shadow_images):
            ...
            # Make prediction
            orig_image, pred_image = make_prediction(G1, G2, shadow_path)
            ...
            # Save a sample of predictions and their histograms (first 10 images)
            if i < 10:
                ...
                # Save the figure
                plt.tight_layout()
                plt.savefig(f"histogram_comparison_stcgan/{checkpoint['name']}_sample_{i+1}.png")
                plt.close()
    ...
    plt.tight_layout()
    plt.savefig("histogram_comparison_stcgan/average_histograms_comparison.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("histogram_comparison_stcgan/combined_average_histograms.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("histogram_comparison_stcgan/input_images_histograms.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("histogram_comparison_stcgan/combined_rgb_histograms.png")
    plt.close()

    print("Histogram comparison completed. Results saved in 'histogram_comparison_stcgan' directory.")
