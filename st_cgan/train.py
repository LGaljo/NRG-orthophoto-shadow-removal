...
os.environ["CUDA_VISIBLE_DEVICES"] = ','.join(list(map(lambda gpu: str(gpu), [0])))
# os.environ["CUDA_VISIBLE_DEVICES"] = '0' # replaced by argument --cuda_device
...
def get_parser():
    ...
    parser.add_argument('-dn', '--dataset_name', type=str, default="TTSD", help='Name of dataset used')
    ...
    parser.add_argument('-cu', '--cuda_device', type=int, default=0, help='cuda device')
    # parser.add_argument('-m', '--mode', type=str, default='all', choices=['all', 'detection', 'removal'], help='Choose which parts of model to train')
...
def plot_log(data, save_model_name='model'):
    ...
    plt.ylabel('loss')
    plt.title('Loss')
    plt.savefig('./logs/' + save_model_name + '_' + initial_time + '.png')
...
def train_model(G1, G2, D1, D2, dataloader, val_dataset, num_epochs, parser, save_model_name='model'):
    ...
    G2.to(device, non_blocking=True)
    D1.to(device, non_blocking=True)
    D2.to(device, non_blocking=True)
    ...
    for epoch in range(num_epochs + 1):
        ...
        print('-----------')
        print('Epoch {}/{}'.format(epoch + load, num_epochs))
        print('(train)')
        ...
        for images, gt_shadow, gt in tqdm(dataloader):
            ...
            gt_shadow = gt_shadow.to(device, non_blocking=True)
            ...
            # L_CGAN1
            label_D1_fake = Variable(torch.tensor(np.zeros(out_D1_fake.size()), device='cuda'), requires_grad=True)
            label_D1_real = Variable(torch.tensor(np.ones(out_D1_fake.size()), device='cuda'), requires_grad=True)
            ...
            # L_CGAN2
            label_D2_fake = Variable(torch.tensor(np.zeros(out_D2_fake.size()), device='cuda'), requires_grad=True)
            label_D2_real = Variable(torch.tensor(np.ones(out_D2_fake.size()), device='cuda'), requires_grad=True)
            ...
            # total
            G_loss = G_L_data1 + lambda_dict['lambda1'] * G_L_data2 + lambda_dict['lambda2'] * G_L_CGAN1 + lambda_dict[
                'lambda3'] * G_L_CGAN2
            G_loss.backward()
            optimizerG.step()
        ...
        print('-----------')
        print('epoch {} || Epoch_D_Loss:{:.4f} || Epoch_G_Loss:{:.4f}'.format(epoch, epoch_d_loss / batch_size,
        ...
        if epoch % 100 == 0 and epoch != 0:
            torch.save(G1.state_dict(), 'checkpoints/' + save_model_name + '_G1_' + str(load + epoch) + '.pth')
            torch.save(G2.state_dict(), 'checkpoints/' + save_model_name + '_G2_' + str(load + epoch) + '.pth')
            torch.save(D1.state_dict(), 'checkpoints/' + save_model_name + '_D1_' + str(load + epoch) + '.pth')
            torch.save(D2.state_dict(), 'checkpoints/' + save_model_name + '_D2_' + str(load + epoch) + '.pth')
            G1.eval()
            G2.eval()
            evaluate(G1, G2, val_dataset, device, '{:s}/val_{:d}'.format('result', epoch))
        if epoch % 10 == 0 and epoch != 0:
            torch.save(G1.state_dict(), 'checkpoints/' + save_model_name + '_G1_tenth' + '.pth')
            torch.save(G2.state_dict(), 'checkpoints/' + save_model_name + '_G2_tenth' + '.pth')
            torch.save(D1.state_dict(), 'checkpoints/' + save_model_name + '_D1_tenth' + '.pth')
            torch.save(D2.state_dict(), 'checkpoints/' + save_model_name + '_D2_tenth' + '.pth')
            G1.eval()
            G2.eval()
            evaluate(G1, G2, val_dataset, device, '{:s}/val_{:d}'.format('result', epoch))
        torch.save(G1.state_dict(), 'checkpoints/' + save_model_name + '_G1_latest' + '.pth')
        torch.save(G2.state_dict(), 'checkpoints/' + save_model_name + '_G2_latest' + '.pth')
        torch.save(D1.state_dict(), 'checkpoints/' + save_model_name + '_D1_latest' + '.pth')
        torch.save(D2.state_dict(), 'checkpoints/' + save_model_name + '_D2_latest' + '.pth')
        G1.eval()
        G2.eval()
        evaluate(G1, G2, val_dataset, device, '{:s}/val_{:d}'.format('result', epoch))
...
# # Parts: D1 and G1
# def train_shadow_detection(G1, G2, D1, dataloader, val_dataset, num_epochs, parser, save_model_name='model'):
#     device = "cuda" if torch.cuda.is_available() else "cpu"
#
#     """use GPU in parallel"""
#     if device == 'cuda':
...
#
#             # total
#             D_loss = lambda_dict['lambda2'] * D_L_CGAN1
#             D_loss.backward()
#             optimizerD.step()
#
#             # Train Generator
...
#
#             # total
#             G_loss = G_L_data1 + lambda_dict['lambda2'] * G_L_CGAN1
#             G_loss.backward()
#             optimizerG.step()
#
#             epoch_d_loss += D_loss.item()
...
#
#         if epoch % 100 == 0:
#             torch.save(G1.state_dict(), 'checkpoints/' + save_model_name + '_G1_' + str(epoch + num_epochs + 1) + '.pth')
#             torch.save(D1.state_dict(), 'checkpoints/' + save_model_name + '_D1_' + str(epoch + num_epochs + 1) + '.pth')
#         elif epoch % 10 == 0:
#             torch.save(G1.state_dict(), 'checkpoints/' + save_model_name + '_G1_tenth' + '.pth')
#             torch.save(D1.state_dict(), 'checkpoints/' + save_model_name + '_D1_tenth' + '.pth')
#         torch.save(G1.state_dict(), 'checkpoints/' + save_model_name + '_G1_latest' + '.pth')
...
#
# # Parts: D2 and G2
# def train_shadow_removal(G1, G2, D2, dataloader, val_dataset, num_epochs, parser, save_model_name='model'):
#
#     device = "cuda" if torch.cuda.is_available() else "cpu"
#
#     """use GPU in parallel"""
...
#     beta1, beta2 = 0.5, 0.999
#
#     optimizerG = torch.optim.Adam([{'params': G2.parameters()}], lr=lr, betas=(beta1, beta2))
#     optimizerD = torch.optim.Adam([{'params': D2.parameters()}], lr=lr, betas=(beta1, beta2))
#
#     criterionGAN = nn.BCEWithLogitsLoss().to(device)
#     criterionL1 = nn.L1Loss().to(device)
#
...
#
#             # Train Discriminator
#             set_requires_grad([D2], True)  # enable backprop$
#             optimizerD.zero_grad()
#
#
#             # for D2
...
#
#             # total
#             D_loss = lambda_dict['lambda3'] * D_L_CGAN2
#             D_loss.backward()
#             optimizerD.step()
#
#             # Train Generator
...
#
#             # total
#             G_loss = lambda_dict['lambda1'] * G_L_data2 + lambda_dict['lambda3'] * G_L_CGAN2
#             G_loss.backward()
#             optimizerG.step()
#
#             epoch_d_loss += D_loss.item()
...
#
#         if epoch % 100 == 0:
#             torch.save(G2.state_dict(), 'checkpoints/' + save_model_name + '_G2_' + str(epoch + num_epochs + 1) + '.pth')
#             torch.save(D2.state_dict(), 'checkpoints/' + save_model_name + '_D2_' + str(epoch + num_epochs + 1) + '.pth')
#         elif epoch % 10 == 0:
#             torch.save(G2.state_dict(), 'checkpoints/' + save_model_name + '_G2_tenth' + '.pth')
#             torch.save(D2.state_dict(), 'checkpoints/' + save_model_name + '_D2_tenth' + '.pth')
#         torch.save(G2.state_dict(), 'checkpoints/' + save_model_name + '_G2_latest' + '.pth')
...
def main(parser):
    ...
    if parser.cuda_device is not None:
        torch.cuda.set_device(parser.cuda_device)

    '''load'''
    if parser.load is not None:
        print('load checkpoint ' + str(parser.load))

        G1_weights = torch.load('./checkpoints/ST-CGAN_G1_' + str(parser.load) + '.pth')
        G1.load_state_dict(fix_model_state_dict(G1_weights))

        G2_weights = torch.load('./checkpoints/ST-CGAN_G2_' + str(parser.load) + '.pth')
        G2.load_state_dict(fix_model_state_dict(G2_weights))

        D1_weights = torch.load('./checkpoints/ST-CGAN_D1_' + str(parser.load) + '.pth')
        D1.load_state_dict(fix_model_state_dict(D1_weights))

        D2_weights = torch.load('./checkpoints/ST-CGAN_D2_' + str(parser.load) + '.pth')
    ...
    check_dir()
    # if parser.mode == 'removal':
    #     train_shadow_removal(G1, G2, D2, dataloader=train_dataloader, val_dataset=val_dataset, num_epochs=num_epochs, parser=parser, save_model_name='ST-CGAN-2')
    # elif parser.mode == 'detection':
    #     train_shadow_detection(G1, G2, D1, dataloader=train_dataloader, val_dataset=val_dataset, num_epochs=num_epochs, parser=parser, save_model_name='ST-CGAN-1')
    # else:
    train_model(G1, G2, D1, D2, dataloader=train_dataloader, val_dataset=val_dataset, num_epochs=num_epochs, parser=parser, save_model_name='ST-CGAN')
