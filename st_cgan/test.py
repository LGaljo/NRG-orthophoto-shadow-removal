...
def get_parser():
    ...
    parser.add_argument('-l', '--load', type=str, default='latest', help='the number of checkpoints')
    # parser.add_argument('-i', '--image_path', type=str, default='./dataset/mine/lay_ao_dof_2019_1155_1277_16.jpg', help='file path of image you want to test')
    # parser.add_argument('-i', '--image_path', type=str, default='./dataset/mine/A/bled-222.jpg', help='file path of image you want to test')
    parser.add_argument('-i', '--image_path', type=str, default='./dataset/mine/A/x750_z500_859319a356172d60-89.jpg', help='file path of image you want to test')
    # parser.add_argument('-i', '--image_path', type=str, default='./dataset/mine/A/bled-57.jpg', help='file path of image you want to test')
...
def test_own_image(G1, G2, path, out_path, size, img_transform):
    ...
    with torch.no_grad():
        ...
        save_image(grid, out_path + '/grid/grid_' + run_time + '_' + path.split('/')[-1])

        detected_shadow = transforms.ToPILImage(mode='L')(unnormalize(detected_shadow)[0, :, :, :])
        detected_shadow = detected_shadow.resize((width, height), Resampling.LANCZOS)
        detected_shadow.save(out_path + '/detected_shadow/detected_shadow_' + run_time + '_' + path.split('/')[-1])

        shadow_removal_image = transforms.ToPILImage(mode='RGB')(unnormalize(shadow_removal_image)[0, :, :, :])
        shadow_removal_image = shadow_removal_image.resize((width, height), Resampling.LANCZOS)
        shadow_removal_image.save(out_path + '/shadow_removal_image/shadow_removal_image_' + run_time + '_' + path.split('/')[-1])
...
def main(parser):
    ...
    if parser.image_path is not None:
        ...
        test_own_image(G1, G2, parser.image_path, parser.out_path, resized_size,
