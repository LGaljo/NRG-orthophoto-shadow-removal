...
class UNetSmaller(Module):
    def __init__(self, nb_classes=config.NUM_CHANNELS):
        super().__init__()

        self.enc1 = Encoder(in_channels=nb_classes, out_channels=32)
        self.enc2 = Encoder(in_channels=32, out_channels=64)
        self.enc3 = Encoder(in_channels=64, out_channels=128)
        self.enc4 = Encoder(in_channels=128, out_channels=256)

        self.bottleneck = Bottleneck(in_channels=256, out_channels=512)

        self.dec4 = Decoder(in_channels=512, out_channels=256)
        self.dec3 = Decoder(in_channels=256, out_channels=128)
        self.dec2 = Decoder(in_channels=128, out_channels=64)
        self.dec1 = Decoder(in_channels=64, out_channels=32)

        self.head = Conv2d(32, nb_classes, 1)
